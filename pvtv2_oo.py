    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=None,
        ...
        for i in range(num_stages):
            if i == 0:
                ...
                patch_embed = ShiftedPatchTokenization(in_dim=embed_dims[i - 1], dim=embed_dims[i], patch_size=2)
            pos_block = PosCNN(embed_dims[i], embed_dims[i])
            ...
            ln = nn.LayerNorm(normalized_shape=[embed_dims[i], c2wh[embed_dims[i]], c2wh[embed_dims[i]]], eps=1e-6)
            cur += depths[i]
...
def pvt_v2_b1(num_classes: int = 1000):
...
def pvt_v2_b2(num_classes: int = 1000):
    model = PyramidVisionTransformerV2(
        patch_size=4,
        embed_dims=[64, 128, 320, 512],
        num_heads=[1, 2, 5, 8],
